...
// ---------- BORSA ----------
const pubEx = new ccxt.binance({ enableRateLimit:true, options:{ defaultType:'spot' } });
let tradeEx = null;
if (CFG.mode !== 'paper') {
...
async function buildUniverse(){
  const tickers = await pubEx.fetchTickers();
  const rows = [];
  for (const sym in tickers){
    if (!sym.endsWith('/'+CFG.quote)) continue;
    const base = sym.slice(0, sym.indexOf('/'));
    const t = tickers[sym];
    const price = t.last, vol = t.quoteVolume||0, c = t.percentage||0;
    if (!price || isStable(base, price, c)) continue;
    rows.push({ base, pair:sym, vol, price, c });
  }
  rows.sort((a,b)=>b.vol-a.vol);
  universe = rows.slice(0, CFG.univMax).map(r=>({ base:r.base, pair:r.pair }));
  for (const r of rows.slice(0, CFG.univMax)){ prices[r.base]=r.price; chg[r.base]=r.c; startPx[r.base]=startPx[r.base]||r.price; }
  log('info','Evren hazir', universe.length+' coin (hacme gore, stablecoin haric).');
}
async function seedHistory(){
  // her coin icin 1m kapanislari cek (indikatorler hemen hazir olsun)
  for (const u of universe){
    try{ const o = await pubEx.fetchOHLCV(u.pair, '1m', undefined, 120); hist[u.base] = o.map(x=>x[4]); }
    catch(e){ hist[u.base] = Array(30).fill(prices[u.base]||1); }
  }
  log('info','Gecmis yuklendi','1 dakikalik mumlarla indikatorler hazir.');
}
async function refreshPrices(){
  const tickers = await pubEx.fetchTickers(universe.map(u=>u.pair));
  for (const u of universe){ const t = tickers[u.pair]; if (t && t.last){ prices[u.base]=t.last; chg[u.base]=t.percentage||chg[u.base]||0;
    if (!hist[u.base]) hist[u.base]=Array(30).fill(t.last); else { hist[u.base].push(t.last); if (hist[u.base].length>220) hist[u.base].shift(); } } }
}
...
(async function start(){
  ...
  try{
    ...
    await buildUniverse(); await seedHistory();
    server.listen(CFG.port, ()=>log('info','Panel hazir', `http://localhost:${CFG.port}  (token ile gir)`));
    setInterval(loop, CFG.pollMs); loop();
  }catch(e){ S.lastError=String(e&&e.message||e); log('warn','Baslatma hatasi', S.lastError); server.listen(CFG.port); }
})();
